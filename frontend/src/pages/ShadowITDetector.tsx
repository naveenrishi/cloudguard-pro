...
// Real data: fetches /api/cloud/accounts/:accountId/resources and flags resources
// with missing required tags (owner, project, env, cost-center) or no owner metadata.
// Falls back to 7 rich demo shadows if API is unreachable.
...
const REQUIRED_TAGS = ['owner', 'project', 'env', 'cost-center'];
...
// ── Demo fallback ─────────────────────────────────────────────────────────
const DEMO_SHADOWS: ShadowResource[] = [
...
// ── Real resource → ShadowResource conversion ────────────────────────────
function toShadow(r: any, i: number): ShadowResource | null {
  const tags: Record<string, string> = r.tags || {};
  const tagKeys = Object.keys(tags).map(k => k.toLowerCase());

  // Find which required tags are missing
  const missingTags = REQUIRED_TAGS.filter(req =>
    !tagKeys.some(k => k === req || k.includes(req))
  );

  // Only treat as shadow if missing ≥2 required tags, or explicitly has no owner
  const hasOwner   = !!(tags['owner'] || tags['Owner'] || tags['CreatedBy'] || tags['created_by'] || tags['team'] || tags['Team']);
  const isShadow   = missingTags.length >= 2 || !hasOwner;

  if (!isShadow) return null;

  const owner = tags['owner'] || tags['Owner'] || tags['CreatedBy'] || tags['created_by'] || null;
  const team  = tags['team']  || tags['Team']  || tags['department'] || null;

  // Risk score: more missing tags + no owner = higher risk
  const riskScore = Math.min(10, missingTags.length * 2 + (!hasOwner ? 2 : 0));

  // Heuristic cost: derive from type (rough monthly estimates)
  const typeL = (r.resourceType || r.type || '').toLowerCase();
  let cost = 15; // default
  if (typeL.includes('ec2') || typeL.includes('instance')) cost = 80 + (i % 5) * 40;
  else if (typeL.includes('rds') || typeL.includes('database')) cost = 150 + (i % 3) * 60;
  else if (typeL.includes('eks') || typeL.includes('cluster'))  cost = 200 + (i % 4) * 80;
  else if (typeL.includes('lambda') || typeL.includes('function')) cost = 8 + i % 12;

  // Provider
  const rawProvider = (r.provider || 'aws').toLowerCase();
  const provider: 'aws' | 'azure' | 'gcp' =
    rawProvider === 'azure' ? 'azure' : rawProvider === 'gcp' ? 'gcp' : 'aws';

  const name = r.name || r.resourceId || `Resource ${i + 1}`;

  // Generate a terraform snippet
  const terraform =
    provider === 'aws' ? `resource "aws_instance" "shadow_${i}" {\n  # Name: ${name}\n  # Owner: UNKNOWN\n  # Add required tags:\n  tags = {\n    Owner       = ""\n    Project     = ""\n    Env         = ""\n    CostCenter  = ""\n  }\n}` : undefined;

  return {
    id:           String(r.resourceId || r.id || `s${i}`),
    name,
    type:         r.resourceType || r.type || 'Resource',
    provider,
    region:       r.region || 'us-east-1',
    cost,
    age:          r.createdAt ? `${Math.floor((Date.now() - new Date(r.createdAt).getTime()) / 86400000)} days` : 'Unknown',
    owner,
    team,
    tags,
    missingTags,
    riskScore,
    lastActivity: r.updatedAt ? new Date(r.updatedAt).toLocaleDateString() : 'Unknown',
    terraform,
  };
}
...
const TYPE_ICONS: Record<string, React.ReactNode> = {
  'ec2': <Server size={14} />, 'rds': <Database size={14} />, 's3': <Cloud size={14} />,
  'lambda': <Cloud size={14} />, 'eks': <Server size={14} />, 'blob': <Cloud size={14} />, 'bigquery': <Database size={14} />,
};
const resolveIcon = (type: string) => {
  const t = (type || '').toLowerCase();
  if (t.includes('ec2') || t.includes('instance')) return <Server size={14} />;
  if (t.includes('rds') || t.includes('database') || t.includes('sql')) return <Database size={14} />;
  if (t.includes('s3') || t.includes('storage') || t.includes('blob')) return <Cloud size={14} />;
  if (t.includes('eks') || t.includes('gke') || t.includes('cluster')) return <Server size={14} />;
  if (t.includes('bigquery') || t.includes('dataset')) return <Database size={14} />;
  return <Cloud size={14} />;
};
const RISK_COLOR = (score: number) => score >= 8 ? '#ef4444' : score >= 6 ? '#f59e0b' : '#6366f1';
...
export default function ShadowITDetector() {
  ...
  const [shadows, setShadows]   = useState<ShadowResource[]>(DEMO_SHADOWS);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/api/cloud/accounts/${accountId}/resources`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) throw new Error('resources failed');

      const data: any    = await res.json();
      const rawResources: any[] = data.resources || data || [];
      if (!rawResources.length) throw new Error('empty');

      const derived = rawResources
        .map((r: any, i: number) => toShadow(r, i))
        .filter(Boolean) as ShadowResource[];

      if (!derived.length) throw new Error('no shadows found');

      setShadows(derived);
      setIsLive(true);
    } catch {
      setShadows(DEMO_SHADOWS);
      setIsLive(false);
    } finally {
      setLoading(false);
    }
  ...
  const filtered = shadows.filter(r =>
    ...
    <MainLayout>
      <div style={{ minHeight: '100vh', background: bg, color: text, fontFamily: "'DM Sans', system-ui, sans-serif", padding: 24 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 13, color: muted, margin: '3px 0 0' }}>
                {loading ? 'Scanning resources…' : isLive
                  ? `${shadows.length} unmanaged resources found (missing ≥2 required tags)`
                  : 'Demo mode · connect account to scan real resources'}
              </p>
            </div>
          </div>
        ...
        <div style={{ position: 'relative', marginBottom: 16 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search shadow resources…"
            style={{ width: '100%', padding: '10px 12px 10px 34px', borderRadius: 12, background: card, border: `1px solid ${border}`, color: text, fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
        </div>
        ...
        {filtered.length === 0 && !loading && (
          <div style={{ textAlign: 'center', padding: 60, background: card, borderRadius: 16, border: `1px solid ${border}` }}>
            ...
            <p style={{ color: text, fontWeight: 600, margin: '0 0 6px' }}>
              {isLive ? 'All resources are properly tagged!' : 'No results match your search.'}
            </p>
            <p style={{ color: muted, fontSize: 13, margin: 0 }}>
              {isLive ? 'Every resource has the required owner, project, env and cost-center tags.' : 'Try a different search term.'}
            </p>
          </div>
        )}
                ...
                <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 16, padding: 18 }}>
                  ...
                  {Object.keys(selected.tags).length > 0 && (
                    <div style={{ marginBottom: 12 }}>
                      <p style={{ fontSize: 12, fontWeight: 600, color: muted, margin: '0 0 6px' }}>Existing Tags</p>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                        {Object.entries(selected.tags).map(([k, v]) => (
                          <span key={k} style={{ fontSize: 11, padding: '2px 8px', borderRadius: 99, background: isDark ? '#1f2937' : '#f3f4f6', color: muted, border: `1px solid ${border}` }}>{k}: {v}</span>
                        ))}
                      </div>
                    </div>
                  )}
