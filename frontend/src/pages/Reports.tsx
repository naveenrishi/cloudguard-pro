...
export default function Reports() {
  ...
  const fetchReports = async () => {
    setLoading(true);
    try {
      const r = await fetch('http://localhost:3000/api/reports', { headers: hdrs });
      if (r.ok) {
  ...
  const handleGenerate = async (type: 'cost'|'security'|'compliance', period: string) => {
    ...
    setGenerating(id);
    try {
      const r = await fetch('http://localhost:3000/api/reports/generate', {
        method: 'POST',
  ...
  const handleDownload = async (report: Report, format: 'pdf'|'csv') => {
    try {
      const r = await fetch(`http://localhost:3000/api/reports/${report.id}/download?format=${format}`, { headers: hdrs });
      if (r.ok) {
  ...
  const handleSchedule = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await fetch('http://localhost:3000/api/reports/schedule', {
        method: 'POST',
        headers: { ...hdrs, 'Content-Type': 'application/json' },
