...
import {
  ...
  Server, Settings, User, Clock, ChevronRight,
  ArrowUpRight, Shield, GitBranch, Activity, X, ArrowLeft,
} from 'lucide-react';
...
const MOCK_CHANGES: Change[] = [
  ...
  { id:'c12', type:'config_drift',      severity:'high',     title:'Azure NSG Rule: Port 3389 Open', description:'RDP port 3389 opened to internet (0.0.0.0/0) on production VM NSG.',                  account:'Azure Dev', provider:'azure', region:'East US',   user:'azureportal',        resource:'nsg-prod-vms',     timestamp:'2026-03-02 11:10' },
];
...
const providerEmoji = (p: string) => p === 'aws' ? '☁️' : p === 'azure' ? '🔷' : '🌐';
...
export default function ChangeInvestigation() {
  ...
  const totalImpact = changes.filter(c => c.type === 'cost_spike').reduce((s, c) => s + (c.costDiff || 0), 0);
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="flex items-start justify-between mb-8">
            <div>
              ...
              <h1 className="text-xl font-bold text-gray-900">Change Investigation</h1>
              <p className="text-xs text-gray-400 mt-0.5">Track what changed, when, who did it, and the cost impact</p>
            </div>
            <button
              onClick={fetchChanges}
              disabled={loading}
              className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 text-gray-600 rounded-xl text-xs font-semibold hover:bg-gray-50 transition-colors shadow-sm disabled:opacity-50"
            >
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-5">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select value={filterType} onChange={e => setFilterType(e.target.value as any)}
                className="px-3 py-2 text-xs font-semibold bg-gray-50 border border-gray-200 rounded-xl focus:outline-none text-gray-600">
          ...
          <div className="space-y-2">
            {filtered.length === 0 ? (
              ...
              filtered.map(change => {
                ...
                return (
                  <div key={change.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <div
                    ...
                    >
                      <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: tc.bg }}>
                        ...
                        <div className="flex items-center gap-3 mt-2 flex-wrap">
                          {change.user && (
                            <span className="flex items-center gap-1 text-xs text-gray-400">
                              <User className="w-3 h-3" />{change.user}
                            </span>
                          )}
                          <span className="flex items-center gap-1 text-xs text-gray-400">
                            <Clock className="w-3 h-3" />{timeAgo(change.timestamp)}
                          </span>
                          {change.costDiff !== undefined && (
                            <span className={`text-xs font-bold flex items-center gap-0.5 ${change.costDiff > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                              <ArrowUpRight className="w-3 h-3" />{fmtDiff(change.costDiff)}/mo
                            </span>
                          )}
                        </div>
                      </div>

                      <ChevronRight className={`w-4 h-4 text-gray-300 flex-shrink-0 mt-1.5 transition-transform ${isExp ? 'rotate-90' : ''}`} />
                    </div>
                      ...
                      <div className="border-t border-gray-100 px-5 py-4 bg-gray-50/40 space-y-4">
                        ...
                        {change.resource && (
                          <div className="flex items-center gap-2 text-xs text-gray-500">
                            <Server className="w-3 h-3" /> Resource:
                            <span className="font-mono font-semibold text-gray-700">{change.resource}</span>
                          </div>
                        )}
                      </div>
                    )}
